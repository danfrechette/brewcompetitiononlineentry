...
<tbody>
 ...
 <tr>
    ...
    <td>
        ...
        <?php //Added by DRF
            if ( $row_dropoff['dropLocationstatus'] ) $icon = "fa-toggle-on";
            else $icon = "fa-toggle-off";

            $_id = $row_dropoff['id'];
            $_go = 'drop-off';
            $_url = sprintf( "href='" . '%1$s' . "includes/ajax_functions.inc.php?action=toggle&go=" . '%2$s' . "&id=" . '%3$d' . "'", $base_url, $_go, $_id);
            $_element =  sprintf("<a " . '%1$s' . " id='togglestate' data-toggle='tooltip' data-placement='top' title='Display On/Off'><span id='t-".'%2$d'."' class='fa fa-lg " . '%3$s' . "'></span></a>", $_url, $_id, $icon );
            echo($_element);
        ?>
