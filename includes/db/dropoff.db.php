...
if (SINGLE) {
    ...
    if (($section == "admin") && ($action == "edit")){
        $query_dropoff .= sprintf(" WHERE id='%s'",$id);
    }
    elseif (($section == "admin") && ($action == "default")) { #Modified By DRF
        $query_dropoff .= " ORDER BY dropLocationName ASC";
    }
    else {
        $query_dropoff .= " WHERE `dropLocationstatus`=1 ORDER BY dropLocationName ASC";
    }
}
